"""Stage 4: FINAL test evaluation of detector v2, one run, fully guarded.

Verifies before doing anything: classifier hash + clean accuracy, detector
hash, and threshold — all against the manifest written by stages 1 and 3.
Test data is used here for the first and only time.
"""
import json
...
def main():
    ...
    assert "detector_v2_sha256" in manifest, "run 03_detector_v2.py first"
    actual = common.sha256_of(d3.DETECTOR_PATH)
    assert actual == manifest["detector_v2_sha256"], (
        "detector file changed since training - mixed-era artifacts refused"
    )
    threshold_record = json.loads(
        d3.THRESHOLD_PATH.read_text(encoding="utf-8"))
    threshold = threshold_record["threshold"]
    assert abs(threshold - manifest["detector_v2_threshold"]) < 1e-12
    detector = tf.keras.models.load_model(d3.DETECTOR_PATH)
    ...
    out = common.RESULTS_DIR / "detector_v2_test_by_epsilon.csv"
    df.to_csv(out, index=False)
    print(df.round(4).to_string(index=False))
    print("saved:", out)
    print("clean test FPR at selected threshold:", round(clean_fpr, 4),
          f"(validation FPR was {threshold_record['validation_fpr']:.4f})")
