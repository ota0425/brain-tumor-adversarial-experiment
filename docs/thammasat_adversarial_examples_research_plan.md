...
- **現在の進捗**：MobileNetV2の学習、通常テスト評価、詳細分類評価、ε = 0, 1, 2, 4, 8のFGSM予備実験まで完了している。
- **次の実験**：ε = 0, 0.01, 0.05, 0.1, 0.25, 0.5, 1の微小摂動領域でFGSMを再評価する。
...
7. **次に実施**：ε = 0, 0.01, 0.05, 0.1, 0.25, 0.5, 1で再実験し、性能低下の立ち上がりを詳細に測定する。
...
|---|---:|---:|---:|---:|---:|---:|---:|
| Accuracy | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち |
| Accuracy Drop | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち |
| Attack Success Rate | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち | 再実行待ち |
...
- εと分類性能の関係を定量的に示せる。（予備実験完了、微小εで再実験待ち）
