...
The score-shift statistics reported in the paper are persisted in
`clean_score_distribution_shift.csv`, generated by Stage 10 on the
verified Colab model.

## เพิ่มเติม 2026-09-02 (dataset audit)

Stages 11--15 were rerun on the final verified model. Their artifacts
remain in `MyDrive/ThammasatResearch/results` and reproduce the audit
values reported in the paper. The duplication count comes only from the
pixel-level scan; the earlier 64-bit perceptual-hash result was refuted
and is not used.
...
The regenerated 300-DPI/vector PDF originals are retained in
`results/figures/` and the two PDFs used by the paper are present here.
...
GPU) แล้วใช้แทน JPEG compact ใน paper. The full-resolution requirement
is therefore closed; the compact JPEG files remain only as transfer
evidence.
