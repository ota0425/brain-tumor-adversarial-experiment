...
this convention can overstate performance when a threshold is transferred
between distinct data collections: thresholds targeting a 10\% FPR
achieve 13.56\% and 15.44\% on the test collection, while
...
requiring labelled attacks. This collection-level result illustrates a
potential risk when a detector is transferred to a hospital with
different scanners or acquisition protocols, although no clinical
deployment was evaluated here. At this fixed operating point, an
...
runs. These findings show why deployment evaluation should report both
discrimination and threshold-specific error rates.
...
This paper asks what happens when an operating point is transferred
between distinct data collections, using a brain-tumor MRI classifier as
a proxy test bed for deployment shift. Our contributions:
\begin{enumerate}
...
\item \textbf{A measured remedy.} Calibrating on deployment-side data
  ...
  attack examples are needed, illustrating a protocol that could use
  the clean images available at a deployment site.
...
fixed-threshold transfer can fail. A DFT-based detector has been proposed for MRI
...
(glioma, meningioma, no-tumor, pituitary) using the Brain Tumor MRI
Dataset distributed by Nickparvar on Kaggle~\cite{nickparvar2021brain}. The
Kaggle data card describes it as a curated combination of the figshare,
SARTAJ, and Br35H collections. The
downloaded archive used in our experiments contained 7{,}200 images:
5{,}600 training and 1{,}600 test images (balanced, resized to
$224\times224$, pixel range 0--255). The training folder is split 80/20 into 4{,}480
...
unflagged ones --- a reminder that a model may fail almost
...
no labelled attacks. This deployment-motivated protocol uses only clean
images from the target collection; it is not a clinical deployment
validation. Deployment-side calibration is standard practice in principle; the
...
On a public medical-imaging dataset, a detector's fixed operating point
failed to transfer between distinct training and test collections even
though its ROC-AUC remained high. Every training-side threshold missed
its FPR budget, while recalibration with 400 clean images from the test
collection --- no attacks required --- brought the FPR close to the
target. This is evidence of a collection-shift risk rather than a
clinical deployment study. Similar shifts may arise when a system is
transferred across hospitals, scanners, or acquisition protocols, so
site-specific validation and calibration should precede clinical use.
We recommend
