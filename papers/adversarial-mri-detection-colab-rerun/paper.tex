...
from the 8.93\% calibration value to 15.44\%. This indicates a shift in
the upper tail of the clean-score distribution, exactly where a fixed
threshold operates. ROC-AUC remains high under the same shift, which is
why AUC-only reporting cannot reveal the problem. A margin variant
(calibrating to 7.5\% for headroom) also misses its target, reaching
14.25\% on the full test set.
